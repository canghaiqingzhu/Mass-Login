...
var P = {
	...
	initAllSelect : function(sysCodeType, sysCodeData) {
		if (sysCodeData) {
			var codeKeys = [];
		...
		function checkBoxSet(id, list) {
			...
			if (P.notEmpty(id) && P.exist(o) && list) {
				...
				$.each(list, function(i) {
					var bean = list[i];
					o.append(checkBox(id, bean['id'], bean['codeName']));
				});
			}
		}
	...
	showSelect : function(id, url, data, faultTitle, faultValue) {
		...
		if (P.notEmpty(id) && P.notEmpty(url) && P.exist(o)) {
			...
			o.empty();
			if (P.notEmpty(faultTitle)) {
				o.append(option('', faultTitle));
			}
			if (json.map) {
	...
	showCheckBox : function(id, url, data, name, notShow) {
		...
		if (P.notEmpty(id) && P.notEmpty(url) && P.exist(o)) {
			...
			if (json.map) {
				$.each(json.map, function(key, value) {
					if (notShow.indexOf(key) == -1) {
						o.append(checkbox(name, key, value));
					}
				});
			} else {
	...
	initDatePicker : function(id, opt, func, callback) {
		var o = $('#' + id);
		if (P.notEmpty(id) && P.exist(o)) {
			var css = [ 'js/public/bootstrapdate/css/bootstrap-datetimepicker.min.css' ];
			P.loadJsOrCss(css, P.getLink);
			var js = [ 'js/public/bootstrapdate/js/bootstrap-datetimepicker.min.js', 'js/public/bootstrapdate/js/locales/bootstrap-datetimepicker.zh-CN.js' ];
			P.loadJsOrCss(js, P.getScript, function() {
				var defaults = {
					/** 语言中文* */
					language : 'zh-CN',
	...
	initBootstrapSelect : function(id, opt, title, callback) {
		...
		if (P.notEmpty(id) && P.exist(o)) {
			...
			P.loadJsOrCss(css, P.getLink);
			var js = [ 'js/public/select2-4.0.3/js/select2.full.min.js' ];
			P.loadJsOrCss(js, P.getScript, function() {
		...
		if (P.exist(o)) {
			$.each(o, function(i) {
				...
				case 'IMG':
					...
					if (signatureCheck.test(key)) {
						showSignature(key, data);
					}
					break;
				case 'P':
					$(o[i]).html(value);
	...
	inArray : function(v, array) {
		if ($.isArray(array) && $.inArray(v, array) != -1) {
			return true;
		}
		return false;
	},
